...
app.get('/', (req, res) => {
    ...
    if (userId === null || userId === undefined) {
        ...
        indexPath =  path.join(staticDir, 'headAdmin.html');
...
app.post('/addRole', async (req, res) => {
    ...
    try {
        ...
        // Вызов функции AddRole
        const result = await connection.execute(
            `BEGIN
                :result := pdbAdmin.AddRole(:p_user_id, :p_name, :p_role_level);
            END;`,
            {
                p_user_id: { val: userId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_name: { val: name, dir: oracledb.BIND_IN, type: oracledb.STRING },
                p_role_level: { val: role_level, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                result: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 }
            }
        );
...
app.post('/deleteRole', async (req, res) => {
    ...
    try {
        ...
        // Вызов функции DeleteRole
        const result = await connection.execute(
            `BEGIN
                :result := pdbAdmin.DeleteRole(:p_user_id, :p_role_id);
            END;`,
            {
                p_user_id: { val: userId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_role_id: { val: roleId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                result: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 }
            }
        );
...
app.post('/updateRole', async (req, res) => {
    const userId = req.session.User_id; // Получаем user_id из сессии
    const { roleId, newName, newRoleLevel } = req.body;
    try {
        if (!userId) {
            return res.status(401).json({ error: 'Пользователь не авторизован' });
        }
        // Вызов функции UpdateRole
        const result = await connection.execute(
            `BEGIN
                :result := pdbAdmin.UpdateRole(:p_user_id, :p_role_id, :p_new_name, :p_new_role_level);
            END;`,
            {
                p_user_id: { val: userId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_role_id: { val: roleId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_new_name: { val: newName, dir: oracledb.BIND_IN, type: oracledb.STRING },
                p_new_role_level: { val: newRoleLevel, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                result: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 }
            }
        );

        // Получаем сообщение из результата
        const message = result.outBinds.result;
...
app.post('/addRole', async (req, res) => {
    const userId = req.session.User_id; // Получаем user_id из сессии
    const { name, role_level } = req.body; // Извлекаем имя и уровень роли из тела запроса
...
app.post('/setRole', async (req, res) => {
    const userId = req.session.User_id; // Получаем user_id из сессии
    const { target_user_id, role_id } = req.body; // Извлекаем target_user_id и role_id из тела запроса
    ...
    try {
        ...
        // Вызов функции SetRole
        const result = await connection.execute(
            `BEGIN
                :result := pdbAdmin.SetRole(:p_user_id, :p_target_user_id, :p_role_id);
            END;`,
            {
                p_user_id: { val: userId, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_target_user_id: { val: target_user_id, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                p_role_id: { val: role_id, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                result: { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 }
            }
        );

        const message = result.outBinds.result; // Получаем сообщение из результата

        res.json({ message }); // Возвращаем сообщение клиенту
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: `Ошибка выполнения запроса: ${err.message}` });
    }
...
app.post('/getUsers', async (req, res) => {
    const userRole = req.body.user_role; // Получаем user_role из тела запроса
    const segment = req.body.segment || 1; // Получаем сегмент из тела запроса, по умолчанию 1

    try {
        // Вызов функции GetUsers
        const result = await connection.execute(
            `BEGIN
                :result := pdbAdmin.GetUsers(:p_user_role, :p_segment);
            END;`,
            {
                p_user_role: { val: userRole, dir: oracledb.BIND_IN, type: oracledb.STRING },
                p_segment: { val: segment, dir: oracledb.BIND_IN, type: oracledb.NUMBER },
                result: { dir: oracledb.BIND_OUT, type: oracledb.CURSOR } // Изменено на CURSOR
            }
        );
        ...
        // Закрываем курсор после завершения
        await cursor.close();

        res.json({ users }); // Возвращаем пользователей в ответе
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: `Ошибка выполнения запроса: ${err.message}` });
    }
